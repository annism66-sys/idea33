...
const brokers = [
  ...
  { id: "upstox", name: "Upstox", logo: "U", color: "from-[#7B2D8E] to-[#5A1F68]", description: "Fast & reliable trading platform", users: "1Cr+ users", popular: true },
  { id: "groww", name: "Groww", logo: "G", color: "from-[#5367FF] to-[#3D4FCC]", description: "Stocks, MFs & more in one app", users: "80L+ users", popular: true },
  { id: "angelone", name: "Angel One", logo: "A", color: "from-[#F26522] to-[#C44F18]", description: "Full-service stockbroker", users: "50L+ users", popular: false },
  { id: "icici", name: "ICICI Direct", logo: "I", color: "from-[#B02A30] to-[#8A2025]", description: "Banking + Broking combined", users: "45L+ users", popular: false },
  { id: "hdfc", name: "HDFC Securities", logo: "H", color: "from-[#004C8F] to-[#003366]", description: "Trusted banking partner", users: "40L+ users", popular: false },
  { id: "kotak", name: "Kotak Securities", logo: "K", color: "from-[#ED1C24] to-[#B8161C]", description: "Award-winning trading platform", users: "30L+ users", popular: false },
  { id: "paytm", name: "Paytm Money", logo: "P", color: "from-[#00BAF2] to-[#0095C4]", description: "Digital-first investing", users: "25L+ users", popular: false },
];

const mockPortfolioData: Record<string, Array<{
  stock_symbol: string; stock_name: string; quantity: number;
  average_price: number; current_price: number; sector: string; exchange: string;
}>> = {
  zerodha: [
...
export function BrokerConnect({ trigger, variant = "default", onConnect }: BrokerConnectProps) {
  ...
  const handleConnect = async (brokerId: string) => {
    if (!user) {
      toast({ title: "Sign in required", description: "Please sign in to connect a broker.", variant: "destructive" });
      return;
    }
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {broker.popular && (
              <span className="px-2 py-0.5 rounded-full bg-primary/10 text-primary text-[10px] font-medium">POPULAR</span>
            )}
          </div>
        ...
        <AnimatePresence mode="wait">
          {isConnected ? (
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-10 h-10 rounded-full bg-gain/10 flex items-center justify-center">
              <Check className="w-5 h-5 text-gain" />
            </motion.div>
          ) : isConnecting ? (
            <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="w-10 h-10 rounded-full border-2 border-primary/30 border-t-primary" />
          ) : (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
    ...
    >
      <div className="flex items-center gap-3">
        ...
        {isConnected ? (
          ...
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="w-4 h-4 rounded-full border-2 border-primary/30 border-t-primary" />
        ) : null}
      </div>
    </motion.div>
  );
